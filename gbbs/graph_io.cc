...
template <>
Edge<gbbs::empty>::Edge(uintE _from, uintE _to) : from(_from), to(_to) {}

std::tuple<size_t, size_t, uintT *, uintE *>
parse_unweighted_graph(const char *fname, bool mmap, bool binary, char *bytes,
                       size_t bytes_size) {
  uintT *offsets;
  uintE *edges;
  uint64_t n, m;
...
read_unweighted_symmetric_graph(const char *fname, bool mmap, bool binary,
...
std::tuple<char *, size_t> parse_compressed_graph(const char *fname,
                                                  bool mmap) {
  char *bytes;
  size_t bytes_size;
