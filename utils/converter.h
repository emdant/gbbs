...
template <class Graph> inline uintE *rankNodes(Graph &GA, size_t n) {
  ...
  for (size_t b = 0; b < n_batches; b++) {
    ...
    parallel_for(start, end, [&](size_t i) {
      ...
      auto write_f = [&](const uintE &ngh, const uintT &offset, const W &val) {
        edges[offset] = std::make_tuple(
            *reinterpret_cast<const uintE *>(&val),
            *reinterpret_cast<const W *>(&ngh)); // workaround for tuples
      };
...
template <class Graph> void edgearray(Graph &GA, std::ofstream &out) {
  using W = typename Graph::weight_type;
  size_t n = GA.n;
  size_t m = GA.m;
...
template <class Graph> auto converter(Graph &GA, commandLine P) {
